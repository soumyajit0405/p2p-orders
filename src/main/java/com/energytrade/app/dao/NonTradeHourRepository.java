...
public interface NonTradeHourRepository extends JpaRepository<NonTradeHour, Long>
{
	  ...
	  @Query(value ="Select sell_order_id from all_sell_orders  where transfer_start_ts >=:startDate and transfer_start_ts <=:endDate union "
	  		+ "Select sell_order_id from all_sell_orders  where transfer_end_ts >=:startDate and transfer_end_ts <=:endDate union 	"
	  		+ "Select sell_order_id from all_sell_orders  where transfer_start_ts <=:startDate and transfer_end_ts >=:endDate and order_status_id <> 2",nativeQuery = true) 
	  List<Integer> getSellOrders( @Param("startDate") String startDate ,@Param("endDate") String endDate);
